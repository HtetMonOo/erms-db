...
CREATE TABLE IF NOT EXISTS `ermsv2`.`town` (
  `pcode` VARCHAR(15) PRIMARY KEY,
  `code` VARCHAR(15) NOT NULL,
  `code_mm` VARCHAR(15) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ermsv2`.`nrc` (
  ...
  `town_pcode` VARCHAR(15) NOT NULL,
  `type` VARCHAR(10) NOT NULL,
  `digit` INT UNSIGNED NOT NULL,
  `date_created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `last_updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY(`town_pcode`) REFERENCES `ermsv2`.`town`(`pcode`) ON DELETE NO ACTION
);
